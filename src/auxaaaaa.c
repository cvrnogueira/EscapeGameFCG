...
float g_originCenterX = 0.0f;
float g_originCenterY = 0.0f;

float Wpressed = false;
float Spressed = false;
float Apressed = false;
float Dpressed = false;
...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - 00274694 - Laura Corssac", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        float y = r*sin(0.0f) + g_cameraCenterY;
        float z = r*cos(0.0f)*cos(0.0f) + g_cameraCenterZ;
        float x = r*cos(0.0f)*sin(0.0f) + g_cameraCenterX;
       ...
       /// printf("phi = %f, theta = %f\n", g_CameraPhi, g_CameraTheta);
        ...
        glm::vec4 camera_lookat_l    = glm::vec4(g_originCenterX,g_originCenterY, g_cameraCenterZ,1.0f); // Ponto "l", para onde a câmera (look-at) estará sempre olhando
...
void CursorPosCallback(GLFWwindow* window, double xpos, double ypos)
{
    ...
    g_originCenterX = -g_CameraTheta;
    g_originCenterY = -g_CameraPhi;
}
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mod)
{
    ...
    //Se o usuário apertar a tecla A, movemos a posição da camera para esquerda
    ...
    //Se o usuário apertar a tecla D, movemos a posição da camera para direita
